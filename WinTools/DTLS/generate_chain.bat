...
:: Generate Root CA
openssl req -addext basicConstraints=critical,CA:TRUE -subj "/CN=RootCA" -new -newkey rsa:2048 -nodes -out ca.csr -keyout ca.key
openssl x509 -trustout -signkey ca.key -days 365 -req -in ca.csr -out ca.crt -extfile v3.ext

:: Generate CA A
openssl genrsa -out CAA.key 2048
...
:: Generate CA B
openssl genrsa -out CAB.key 2048
